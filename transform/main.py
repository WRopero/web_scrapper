...
def main(filename):
    ...
    df = _extract_host(df)
    df = _remove_slash_at_end_from_url(df)
...
def _remove_slash_at_end_from_url(df):
    logger.info('Remove slash at end from url')
    
    stripped_url = df.apply(__filter_by_slash, axis=1)
    selected_urls = df[stripped_url]['url'].apply(lambda url: url[:-1])
    df.loc[stripped_url, 'url'] = selected_urls
    return df

def __filter_by_slash(row):
    if row['url'][-1] == '/':
        return True
    else:
        return False
